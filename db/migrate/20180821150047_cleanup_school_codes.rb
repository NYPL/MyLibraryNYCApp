  ...
  def up
    ...
    ActiveRecord::Base.transaction do
      ...
      school_id_array.each do |school_id_pair|
        ...
        Rails.logger.debug "IDs: #{[school_to_merge.id, school_to_keep.id]} Campus IDs: #{[school_to_merge.campus_id, school_to_keep.campus_id]}"
      ...
      Rails.logger.debug "***************** School Cleanup 2"
      School.all.each do |school|
        Rails.logger.debug school.code
        school.code = school.code.downcase.gsub('zz', 'z').strip
        school.save
      end

      # delete one duplicate school with an extra space in zcode
      Rails.logger.debug "***************** School Cleanup 3"
      s = School.find_by(code: "zk681 ")

      if s && s.users.count == 0
        s.destroy
      else
        Rails.logger.debug "Not deleting school with zcode: zk681 & space on end"
      end

      # delete schools not found in data.gov
      Rails.logger.debug "***************** School Cleanup 4"
      ...
      codes_for_schools_to_delete.each do |code_for_school_to_delete|
        s = School.find_by(code: code_for_school_to_delete)
        ...
        if s
          ...
          Rails.logger.debug "Deleting school with this code: #{code_for_school_to_delete}."
          s.destroy
        else
          Rails.logger.debug "Couldn't find school with this code: #{code_for_school_to_delete}."
        end
      end
    end
  end
