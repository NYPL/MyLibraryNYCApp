...
class RoutesTest < ActionDispatch::IntegrationTest
  ...
  test "test for sets index page with sets hostname" do
    assert_routing "http://#{ENV['MLN_SETS_SITE_URL']}", :controller => "home", :action => "index"
  end
  ...
  test "test for info-site participating-schools page with sets hostname" do
    resp = assert_raises ActionController::RoutingError do
      Rails.application.routes.recognize_path("http://#{ENV['MLN_SETS_SITE_URL']}/about/participating-schools")
    end
  ...
  test "test for info-site about page with sets hostname" do
    resp = assert_raises ActionController::RoutingError do
      Rails.application.routes.recognize_path("http://#{ENV['MLN_SETS_SITE_URL']}/about/about-mylibrarynyc")
    end
    assert_equal(true, resp.message.include?(NO_ROUTE_MATCHES))
  end
  ...
  test "test for info-site contacts-links page with sets hostname" do
    resp = assert_raises ActionController::RoutingError do
      Rails.application.routes.recognize_path("http://#{ENV['MLN_SETS_SITE_URL']}/contacts-links")
    end
    assert_equal(true, resp.message.include?(NO_ROUTE_MATCHES))
  end
  ...
  test "test for info-site digital resources page page with sets hostname" do
    resp = assert_raises ActionController::RoutingError do
      Rails.application.routes.recognize_path("http://#{ENV['MLN_SETS_SITE_URL']}/help/access-digital-resources")
    end
    assert_equal(true, resp.message.include?(NO_ROUTE_MATCHES))
  end
