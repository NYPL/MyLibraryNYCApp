...
class SessionsController < ApplicationController
  ...
  def timeout_check
    ...
    if !session || !session["warden.user.user.session"]
      render json: { 'timeout_status': 'no_current_user' }, status: :continue
    elsif session["warden.user.user.session"] && Time.zone.now.to_i > timeout_timestamp.to_i
      sign_out current_user
      flash[:notice] = "Your session has timed out."
      render json: { 'timeout_status': 'timed_out' }, status: :ok
    elsif session["warden.user.user.session"] && Time.zone.now.to_i > timeout_warning_timestamp.to_i
      render json: { 'timeout_status': 'timeout_warning' }, status: :ok
    else
      render json: { 'timeout_status': 'not_timed_out' }, status: :ok
    end
  end
