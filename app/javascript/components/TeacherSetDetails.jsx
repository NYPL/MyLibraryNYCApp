...
import {
  ...
  Text, Form, FormField, SimpleGrid, ButtonGroup, Box, HorizontalRule, StatusBadge, VStack, Icon, Breadcrumbs, Hero, Notification, useNYPLBreakpoints, Accordion
...
export default function TeacherSetDetails(props) {
  ...
  const TeacherSetDescription = () => {
    return <Text noSpace marginTop="xs" id="ts-page-desc">{teacherSet["description"]}</Text>
  }

  const AvailableCopies = () => {
    return <div color="var(--nypl-colors-ui-black)">{teacherSet["available_copies"]} of {teacherSet["total_copies"]} Available</div>
  }
  ...
  const TeacherSetBooks = () => {
    return books.map((data, i) => {
      return <><ReactRouterLink onClick={ () => window.scrollTo(0, 0) } id={"ts-books-" + i} to={"/book_details/" + data.id} >
            {BookImage(data)}
        </ReactRouterLink></>
    })
  }
  ...
  const teacherSetAvailability = () => {
    if (isLargerThanMobile && teacherSet.availability !== undefined) {
      return <StatusBadge level={availabilityStatusBadge()}>{titleCase(teacherSet.availability)}</StatusBadge>
    } else {
      return null
    }
  }
  ...
  const orderTeacherSetsInfo = () => {
    if (isLargerThanMobile || (!isLargerThanMobile && ((teacherSet && teacherSet.available_copies > 0) && allowedQuantities.length > 0) )) {
